...
def recurse_maximal_substring(
    string1, string2, string1_idx, string2_idx, substring, max_substring
):
    # string1_idx = 0
    # string2_idx = 0
    # substring = ""
    ...
    for idx in range(string1_idx, len(string1)):
        if string1[idx] == string2[string2_idx]:
            cur_substring = recurse_maximal_substring(
                string1,
                string2,
                idx + 1,
                string2_idx + 1,
    ...
    cur_substring = recurse_maximal_substring(
        string1, string2, string1_idx + 1, string2_idx + 1, substring, max_substring
    )
    if len(cur_substring) > len(max_substring):
        max_substring = cur_substring
    return max_substring
